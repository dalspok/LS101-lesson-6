...
def joinor(arr, default_joiner = ", ", end_joiner = "or")
  last = arr.pop
  return last.to_s if arr.empty?
  arr.join(default_joiner) + default_joiner + end_joiner + ' ' + last.to_s
end
...
def computer_places_piece!(board)
  key = if third_square_in_row(board, COMPUTER_MARKER)
          third_square_in_row(board, COMPUTER_MARKER)
        elsif third_square_in_row(board, PLAYER_MARKER)
...
def check_starting(who_starts_game)
  system "clear"
  prompt "Welcome to Tic-Tac-Toe. Who wins 5 games, takes the price."
